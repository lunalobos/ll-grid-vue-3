...
function compareValue(a, b) {
    if (a.type !== b.type) {
        const x = new Value(new String(a.value).toString());
        const y = new Value(new String(b.value).toString());
...
export class Value {
     ...
     * @param {string} format
     */
    constructor(value, type = 'string', format = 'es-Ar') {
        this.value = value;
        this.type = type;
        this.format = format;
    }
    ...
    equals(other) {
        ...
        if (other instanceof Value) {
            return (this.value === other.value) && (this.format === other.format) && (this.type === other.type);
        } else {
            return false;
        }
    }
    ...
    compareTo(other) {
        if (this.type !== other.type) {
            const a = new Value(new String(this.value).toString());
            const b = new Value(new String(other.value).toString());
...
export class Row {
    ...
    constructor(cells) {
        this.cells = cells;
        this.cellsMap = {};
        this.cells.filter(cell => cell.buttonInfo === null).forEach(cell => {
...
function toTableProps(models, types, formats = {}) {
    ...
    for (let i = 0; i < models.length; i++) {
        ...
        for (let j = 0; j < keys.length; j++) {
            ...
            const cell = format ? new Cell(key, new Value(value, type, format)) : new Cell(key, new Value(value, type));
            cells.push(cell);
        }
...
function rowsToProps(rows) {
    ...
    if (rows.length > 0) {
        ...
        for (let i = 0; i < output.headers.length; i++) {
            ...
            let allValues = rows.map(row => row.cells.find(cell => cell.key === header.name).value);
...
function calculateWidths(headers) {
    const columnWidths = headers.map(header => {
        ...
        const headerWidths = header.values.values.map(value => magicFunction(value.formattedValue().length));
...
export class Table {
     ...
     * @param {*} models 
     * @param {Array<string>} types 
     * @param {Array<number>} pagination
     * @param {Array<string>} formats
     */
    constructor(name, models, types, pagination = [10, 20, 50], formats = {}) {
        this.change = shallowReactive({ content: new Date(), filters: new Date() });
        this.setContent(name, models, types, formats, pagination);
    }
     ...
     * @returns {boolean}
     */
    setContent(name, models, types, formats = {}, pagination) {
        this.name = name;
        const { rows, headers, headersModel } = toTableProps(models, types, formats);
        this.rows = rows;
        ...
        this.page = 0;
        this.pages = this.pageSize > 0 ? Math.ceil(this.rows.length / this.pageSize) : 1;
     ...
     */
    removeFilter(key) {
        if (this.headers.some(header => header.name === key) && this.filters[key]) {
    ...
    sortByField(field, order) {
        switch (order) {
            case 'up':
                 ...
                 */
                this.sortBy.sorter = (r1, r2) => r1.getCell(field).value.compareTo(r2.getCell(field).value);
                this.change.content = new Date();
                return true;
            case 'down':
                /**
    ...
    visibleRows() {
        ...
        if (this.pageSize > 0) {
            ...
            const to = this.page * this.pageSize + this.pageSize; // exclusive
