<template>
    <div class="border border-gray-400 rounded-xl">
        ...
        <div class="overflow-y-auto lg:max-h-[400px] md:max-h-[300px] sm:max-h-[200px]">
            <div v-for="(row, rowNum) in rows" class="grid"
                ...
                <div v-for="cell in row.cells" class="p-1"
                    :style="`background-color: ${isOdd(rowNum) ? 'oklch(87.2% 0.01 258.338)' : 'oklch(96.7% 0.003 264.542)'}; grid-column: span ${widthInfo.widths[cell.key]} / span ${widthInfo.widths[cell.key]};`">
                    <div v-if="cell.buttonInfo" class="relative flex flex-row text-sm items-center justify-center">
                        <button class="flex flex-row p-2 cursor-pointer rounded-lg bg-gray-400 hover:bg-gray-500 text-white"
        ...
        <div class="flex flex-row justify-normal p-4 rounded-b-xl bg-gray-200 ">
            ...
            <button class="rounded-lg hover:bg-gray-300 cursor-pointer p-1"
            ...
            <button class="rounded-lg hover:bg-gray-300 cursor-pointer p-1" @click="() => props.getter().nextPage()">
                <ChevronRight />
            </button>
            <button class="rounded-lg hover:bg-gray-300 cursor-pointer p-1" @click="() => props.getter().lastPage()">
                <ChevronDoubleRight />
            </button>
        </div>
    </div>
...
<script setup>
import GridHeaderGrayMonochrome from './GridHeaderGrayMonochrome.vue';
